...
// --- WORD ARRAYS (keep them here as requested) ---
const WORDS = {
...
// --- GAME STATE (in-memory; resets when server restarts or admin re-joins) ---
let STATE = null;

function resetStateKeepPlayers() {
  const prevPlayers = (STATE && STATE.players) || {};
  STATE = {
    players: prevPlayers, // keep connected clients' basic objects if any
    lobby: Object.keys(prevPlayers),
    teams: { team1: [], team2: [] },
    teamNames: { team1: 'Equipe 1', team2: 'Equipe 2' },
    adminId: null,
    phase: 'join',
    currentCategory: null,
    categoryChosen: new Set(),
    usedWords: new Set(),
    scores: { team1: 0, team2: 0 },
    roundWords: [],
    rotationIndex: { team1: 0, team2: 0 },
    currentTurn: { team: 'team1' },
    chosenPlayer: null, // socketId
    timer: null,
    timerRemaining: 0
  };
}
...
io.on('connection', (socket) => {
  ...
  socket.on('join', ({ name, isAdmin }) => {
    // If admin joins, reset the game state (scores, used words, etc.) as requested
    if (isAdmin) {
      // clear all except players list
      resetStateKeepPlayers();
      STATE.adminId = socket.id;
      STATE.players[socket.id] = { name, isAdmin: true, socketId: socket.id, team: null };
      // move admin to lobby
      STATE.lobby = Object.keys(STATE.players);
      io.emit('state', getPublicState());
      return;
    }

    // visitor
    STATE.players[socket.id] = { name, isAdmin: false, socketId: socket.id, team: null };
    STATE.lobby = Object.keys(STATE.players).filter(id => !STATE.teams.team1.includes(id) && !STATE.teams.team2.includes(id));
    io.emit('state', getPublicState());
  });

  socket.on('makeAdmin', ({ password }) => {
    if (password === '12345678') {
      // grant admin to this socket and reset state
      resetStateKeepPlayers();
      STATE.adminId = socket.id;
      if (!STATE.players[socket.id]) STATE.players[socket.id] = { name: 'Admin', isAdmin: true, socketId: socket.id, team: null };
  ...
  socket.on('moveToTeam', ({ playerId, team }) => {
    if (socket.id !== STATE.adminId) return; // only admin
    ...
    // add to team
    if (team === 'team1') STATE.teams.team1.push(playerId);
    if (team === 'team2') STATE.teams.team2.push(playerId);
    // update players
    if (STATE.players[playerId]) STATE.players[playerId].team = team;
    io.emit('state', getPublicState());
  });

  socket.on('moveToLobby', ({ playerId }) => {
    if (socket.id !== STATE.adminId) return;
    STATE.teams.team1 = STATE.teams.team1.filter(id => id !== playerId);
    STATE.teams.team2 = STATE.teams.team2.filter(id => id !== playerId);
    if (!STATE.lobby.includes(playerId)) STATE.lobby.push(playerId);
    if (STATE.players[playerId]) STATE.players[playerId].team = null;
    io.emit('state', getPublicState());
  });
  ...
  socket.on('startCategories', () => {
    ...
    io.emit('phase', { phase: STATE.phase });
  });

  socket.on('selectCategory', ({ category }) => {
    if (socket.id !== STATE.adminId) return;
    if (STATE.categoryChosen.has(category)) return;
    STATE.currentCategory = category;
    STATE.categoryChosen.add(category);

    // prepare first team (team1) player
    STATE.currentTurn.team = 'team1';
    const teamArr = STATE.teams.team1;
    if (!teamArr || teamArr.length === 0) {
      socket.emit('noPlayers');
      return;
    }
    const idx = STATE.rotationIndex.team1 % teamArr.length;
    STATE.chosenPlayer = teamArr[idx];

    STATE.phase = 'prepare';
    io.emit('categoryChosen', { category, phase: STATE.phase });
    // notify chosen player that they are selected
    if (STATE.chosenPlayer) {
      io.to(STATE.chosenPlayer).emit('youAreChosen', { name: STATE.players[STATE.chosenPlayer].name });
    }
  });

  socket.on('beginTurn', () => {
    if (socket.id !== STATE.chosenPlayer) return;
    // start round server-side
    startRound();
  });

  socket.on('gotIt', () => {
    // only chosen player can press
    if (socket.id !== STATE.chosenPlayer) return;
    if (!STATE.currentWord) return;
    STATE.roundWords.push({ word: STATE.currentWord, status: 'acertou', team: STATE.currentTurn.team });
    // award point
    STATE.scores[STATE.currentTurn.team] += 1;
    STATE.usedWords.add(STATE.currentWord);
    pickNextWordOrEndIfNone();
    io.emit('roundUpdate', getRoundPublic());
  });

  socket.on('skip', async () => {
    if (socket.id !== STATE.chosenPlayer) return;
    if (!STATE.currentWord) return;
    STATE.roundWords.push({ word: STATE.currentWord, status: 'pulou', team: STATE.currentTurn.team });
    STATE.usedWords.add(STATE.currentWord);
    pickNextWordOrEndIfNone();
    io.emit('roundUpdate', getRoundPublic());
  });

  socket.on('endVerifyNext', () => {
    if (socket.id !== STATE.adminId) return;
    // If team1 just played, start team2; if team2 just played, return to categories and update rotation
    if (STATE.currentTurn.team === 'team1') {
      // start team2
      STATE.currentTurn.team = 'team2';
      const teamArr = STATE.teams.team2;
      if (!teamArr || teamArr.length === 0) {
        STATE.phase = 'categories';
        io.emit('phase', { phase: STATE.phase, categoryChosen: Array.from(STATE.categoryChosen) });
        return;
      }
      const idx = STATE.rotationIndex.team2 % teamArr.length;
      STATE.chosenPlayer = teamArr[idx];
      STATE.phase = 'prepare';
      io.emit('categoryChosen', { category: STATE.currentCategory, phase: STATE.phase });
      if (STATE.chosenPlayer) io.to(STATE.chosenPlayer).emit('youAreChosen', { name: STATE.players[STATE.chosenPlayer].name });
    } else {
      // team2 finished -> go back to categories and rotate
      STATE.rotationIndex.team1 = (STATE.rotationIndex.team1 + 1) % Math.max(1, STATE.teams.team1.length);
      STATE.rotationIndex.team2 = (STATE.rotationIndex.team2 + 1) % Math.max(1, STATE.teams.team2.length);
  ...
  socket.on('finalizeConfirm', ({ confirm }) => {
    if (socket.id !== STATE.adminId) return;
    if (confirm) {
      STATE.phase = 'final';
      io.emit('phase', { phase: STATE.phase, finalScores: STATE.scores });
    } else {
      STATE.phase = 'categories';
      io.emit('phase', { phase: STATE.phase, categoryChosen: Array.from(STATE.categoryChosen) });
    }
  ...
  socket.on('disconnect', () => {
    ...
    if (socket.id === STATE.adminId) {
      // admin left -> clear admin and reset
      STATE.adminId = null;
      // keep players but move everyone to lobby
      STATE.lobby = Object.keys(STATE.players);
      STATE.phase = 'join';
...
function getPublicState() {
  return {
    lobby: STATE.lobby.map(id => ({ id, name: STATE.players[id] ? STATE.players[id].name : 'unknown', team: STATE.players[id] ? STATE.players[id].team : null })),
    teams: {
      team1: STATE.teams.team1.map(id => ({ id, name: STATE.players[id] ? STATE.players[id].name : 'unknown' })),
      team2: STATE.teams.team2.map(id => ({ id, name: STATE.players[id] ? STATE.players[id].name : 'unknown' }))
    },
    teamNames: STATE.teamNames,
    phase: STATE.phase,
    adminId: STATE.adminId,
    scores: STATE.scores,
    categoryChosen: Array.from(STATE.categoryChosen),
...
function startRound() {
  STATE.roundWords = [];
  STATE.timerRemaining = 75;
  STATE.phase = 'playing';
  pickNextWordOrEndIfNone(true);

  io.emit('roundStart', getRoundPublic());

  if (STATE.timer) clearInterval(STATE.timer);
  STATE.timer = setInterval(() => {
    STATE.timerRemaining -= 1;
    io.emit('timer', { t: STATE.timerRemaining });
    if (STATE.timerRemaining <= 0) {
      clearInterval(STATE.timer);
      STATE.timer = null;
      STATE.phase = 'verify';
      io.emit('roundEnded', { roundWords: STATE.roundWords, scores: STATE.scores });
    }
  }, 1000);
}

function pickNextWordOrEndIfNone(first=false) {
  const cat = STATE.currentCategory;
  if (!cat) return;
  const pool = (WORDS[cat] || []).filter(w => !STATE.usedWords.has(w));
  if (pool.length === 0) {
    STATE.currentWord = null;
    return;
  }
  const randomIdx = Math.floor(Math.random() * pool.length);
  STATE.currentWord = pool[randomIdx];
  if (!first) {
    // don't immediately mark used until scored or skipped
  }
}