...
// servidor simples com Express + Socket.IO
...
// --- jogo: estado em memória (volátil) ---
const CATEGORIES = {
...
let state = {
  players: [], // {id, name, role: 'visitor'|'admin', socketId}
  lobby: [], // arrays of player ids
  team1: [],
  team2: [],
  ...
  current: {
    category: null,
    teamTurn: null, // 1 or 2
    currentPlayerId: null,
    roundWords: [], // [{word, result: 'hit'|'skip'}]
  },
...
function resetGameState() {
  ...
  state.current = { category: null, teamTurn: null, currentPlayerId: null, roundWords: [] };
  state.nextIndex = { team1: 0, team2: 0 };
  // keep players in lobby/teams as-is
}
...
function broadcastState() {
  ...
  io.emit('state', {
    ...
    team2: state.team2,
    scores: state.scores,
    usedCategories: Array.from(state.usedCategories),
    current: { category: state.current.category, teamTurn: state.current.teamTurn, currentPlayerId: state.current.currentPlayerId },
  });
}
...
function findPlayerBySocket(id) {
  return state.players.find(p => p.socketId === id);
}
...
io.on('connection', socket => {
  ...
  socket.on('join', ({ name, role, password }) => {
    // role: 'visitor' or 'admin' from client; verify password
    let finalRole = 'visitor';
    if (role === 'admin' && password === '12345678') finalRole = 'admin';

    // If an admin connects, we reset the game state (as requested)
    if (finalRole === 'admin') {
      resetGameState();
      console.log('Admin connected — game state reset');
    }

    const player = { id: socket.id, name: name || 'Anon', role: finalRole, socketId: socket.id };
    state.players.push(player);
    state.lobby.push(player.id);

    socket.emit('joined', { ok: true, id: player.id, role: finalRole });
    broadcastState();
  });

  socket.on('move-player', ({ playerId, target }) => {
    const admin = findPlayerBySocket(socket.id);
    if (!admin || admin.role !== 'admin') return;

    // remove from whichever list
    [state.lobby, state.team1, state.team2].forEach(arr => {
      const i = arr.indexOf(playerId);
      if (i !== -1) arr.splice(i, 1);
    });

    if (target === 'lobby') state.lobby.push(playerId);
    else if (target === 'team1') state.team1.push(playerId);
    else if (target === 'team2') state.team2.push(playerId);

    broadcastState();
  });

  socket.on('start-category', ({ category }) => {
    const admin = findPlayerBySocket(socket.id);
    if (!admin || admin.role !== 'admin') return;
    if (!CATEGORIES[category]) return;
    state.usedCategories.add(category);
    state.current.category = category;
    state.current.roundWords = [];
    // start with team 1 per spec
    state.current.teamTurn = 1;
    // pick next player from team1 using rotational index
    const playerId = getNextPlayerIdForTeam(1);
    state.current.currentPlayerId = playerId;
    io.emit('prepare-player', { playerId, category });
    broadcastState();
  });
  ...
  socket.on('start-turn', () => {
    ...
    if (!player) return;
    if (player.id !== state.current.currentPlayerId) return; // only chosen player can start

    // start server-side timer that broadcasts every second
    let timeLeft = 75; // seconds
    io.emit('turn-start', { timeLeft });

    const tick = setInterval(() => {
      timeLeft -= 1;
      io.emit('time-tick', { timeLeft });
      if (timeLeft <= 0) {
        clearInterval(tick);
        // end turn
        io.emit('turn-end', { roundWords: state.current.roundWords, scores: state.scores });
      }
    }, 1000);

    // store timer to be able to cancel if needed
    state.timers.currentTick = tick;

    // send first word
    const firstWord = pickWordFromCategory(state.current.category);
    if (!firstWord) {
      io.to(player.socketId).emit('no-words');
      return;
    }
    io.to(player.socketId).emit('word', { word: firstWord });
    io.emit('time-tick', { timeLeft });
  });

  socket.on('action', ({ type, word }) => {
    const player = findPlayerBySocket(socket.id);
    if (!player) return;
    if (player.id !== state.current.currentPlayerId) return; // only active player

    if (type === 'hit') {
      // add point to current team
      if (state.current.teamTurn === 1) state.scores.team1 += 1;
      else state.scores.team2 += 1;
      state.current.roundWords.push({ word, result: 'hit' });
      // pick next word
      const next = pickWordFromCategory(state.current.category);
      if (next) io.to(player.socketId).emit('word', { word: next });
      else io.to(player.socketId).emit('no-words');
      io.emit('scores', state.scores);
    } else if (type === 'skip') {
      // log skip
      state.current.roundWords.push({ word, result: 'skip' });
      // inform all clients that skipping started
      io.emit('skipping', { word });
      // 3 seconds pause, but time continues on server
      setTimeout(() => {
        const next = pickWordFromCategory(state.current.category);
        if (next) io.to(player.socketId).emit('word', { word: next });
        else io.to(player.socketId).emit('no-words');
  ...
  socket.on('advance-after-verification', () => {
    const admin = findPlayerBySocket(socket.id);
    if (!admin || admin.role !== 'admin') return;
    // if current team was 1, go to team 2 with same category
    if (state.current.teamTurn === 1) {
      state.current.teamTurn = 2;
      state.current.roundWords = [];
      const playerId = getNextPlayerIdForTeam(2);
      state.current.currentPlayerId = playerId;
      io.emit('prepare-player', { playerId, category: state.current.category });
      broadcastState();
    } else {
      // both teams played -> back to categories. Hide this category (already in usedCategories)
      state.current = { category: null, teamTurn: null, currentPlayerId: null, roundWords: [] };
      broadcastState();
    }
  });

  socket.on('finalize-game', () => {
    const admin = findPlayerBySocket(socket.id);
    if (!admin || admin.role !== 'admin') return;
    io.emit('game-final', { scores: state.scores });
  });

  socket.on('disconnect', () => {
    // remove player
    const i = state.players.findIndex(p => p.socketId === socket.id);
    if (i !== -1) {
      const pid = state.players[i].id;
      state.players.splice(i, 1);
      [state.lobby, state.team1, state.team2].forEach(arr => {
        const j = arr.indexOf(pid);
...
function getNextPlayerIdForTeam(teamNumber) {
  const arr = teamNumber === 1 ? state.team1 : state.team2;
  if (!arr || arr.length === 0) return null;
  const idx = state.nextIndex[teamNumber === 1 ? 'team1' : 'team2'] % arr.length;
  const pid = arr[idx];
  state.nextIndex[teamNumber === 1 ? 'team1' : 'team2'] = (state.nextIndex[teamNumber === 1 ? 'team1' : 'team2'] + 1) % arr.length;
  return pid;
}
